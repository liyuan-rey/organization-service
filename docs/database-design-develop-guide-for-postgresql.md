...
4. It is recommended to add `create_time` and `update_time` fields to main business tables for optimistic locking mechanisms and troubleshooting.
...
1. Table names should follow specific prefix conventions: `org_<table_name>`, in which `org_` is prefix.
...
- `update_time`: Record last update time, automatically updated using Spring JPA's built-in optimistic locking mechanism
