...
- 使用中文，特别是在文档，注释，Git 提交信息，交互对话等地方
- 英文技术术语（如 Java、Spring Boot、RESTful API、DTO、JPA、MapStruct 等等）使用英文原文
- 中文和英文混排时确保留有一个空格
...
- 数据库表主键采用 `UUID` 类型，主键在 Java 代码中生成，生成使用工具类方法 `UUIDv7.randomUUID()`
...
- 统一返回JSON结构：`{status: integer, message: string, data: Object | Array<any>}`
  ...
  - message：对应状态码，以文本描述对请求的响应结果，例如 "成功"、"失败"、"参数错误" 等
