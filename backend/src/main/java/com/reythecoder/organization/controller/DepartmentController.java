...
public class DepartmentController {
    ...
    @GetMapping
    public ApiResult<List<DepartmentRsp>> getAllDepartments() {
    ...
    @GetMapping("/{id}")
    public ApiResult<DepartmentRsp> getDepartmentById(@PathVariable UUID id) {
    ...
    @PutMapping("/{id}")
    public ApiResult<DepartmentRsp> updateDepartment(@PathVariable UUID id,
        ...
        return ApiResult.success("部门更新成功", department);
    }

    @DeleteMapping("/{id}")
    @ResponseStatus(HttpStatus.NO_CONTENT)
    public ApiResult<Void> deleteDepartment(@PathVariable UUID id) {
        logger.info("收到删除部门请求: {}", id);
        departmentService.deleteDepartment(id);
        return ApiResult.success("部门删除成功", null);
    }
}
